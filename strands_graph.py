...
class TrieNode:
    def __init__(self, extra=None, count=0) -> None:
    ...
    def str_aux(self):
        ...
        if self.A is not None:
            ...
            #sub_tree.append(" " * len(sub_tree[-1]))
            lines += A_lines
    ...
    def search(self, strand: str, allow_mis_matches=0) -> Vertex | None:
        node = self
...
class Graph:
    ...
    def load_from_strands(self, strands: list[str], allow_mis_matches) -> None:
        trie = TrieNode()
    ...
    def get_sequenced_result(self, min_overlap=7, in_coming_links_min=40) -> str:
        incoming_links_values = {v: 0 for v in self.vertices.keys()}
        #print(f"{len(self.vertices.keys())=}")
